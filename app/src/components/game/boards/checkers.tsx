...
export default function Checkers({ lobby, isSpectator, turn, endTurn }: Props) {
    ...
    const getWinner = useCallback(
        (board: Board): "creator" | "opponent" | null => {
            const flatBoard = board.flat();
            const creatorPieces = flatBoard.filter((p) =>
                p.includes("creator")
            ).length;
            const opponentPieces = flatBoard.filter((p) =>
                p.includes("opponent")
            ).length;
    ...
    const handleCellClick = useCallback(
        async (row: number, col: number) => {
            ...
            if (board[row][col].includes(currentSide)) {
                const jumpSource = allJumps.find(
                ...
                setValidTargets(
                    jumpSource ? jumpSource.targets : getValidMoves(row, col)
                );
                return;
            }
            ...
            if (
                ...
                if (allJumps.length > 0 && Math.abs(row - fromRow) !== 2)
                    return;
    ...
    return (
        <div className="flex flex-col items-center p-4">
            <div
                className={cn(
                    "grid gap-1 bg-[#19327a] p-4 border-2 border-[#00bafe] rounded-3xl",
                                ...
                                >
                                    {cell != "none" && (
                                        <Image
                                            ...
                                            src={`/images/games/checkers/${cell.replaceAll(
                                                "-king",
                                                ""
                                            )}.png`}
                                            alt={cell}
