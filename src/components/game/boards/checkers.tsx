...
const initialBoard: Board = Array(8).fill('none').map(() => Array(8).fill('none'));
...
export default function Checkers({ lobby, turn, endTurn }: Props) {
  ...
  const isValidMove = useCallback((fromRow: number, fromCol: number, toRow: number, toCol: number) => {
  ...
  const handleCellClick = useCallback(async (row: number, col: number) => {
    ...
    if (selectedPiece.current && isValidMove(selectedPiece.current.row, selectedPiece.current.col, row, col)) {
      ...
      if ((currentSide === 'creator' && row === 0) || (currentSide === 'opponent' && row === 7)) {
              ...
              >
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center ${
                    cell === 'creator' ? 'bg-blue-600' :
                    cell === 'creator-king' ? 'bg-blue-800' :
                    cell === 'opponent' ? 'bg-orange-600' :
                    cell === 'opponent-king' ? 'bg-orange-800' :
                    'bg-transparent'
                  }`}
                >
                  {cell === 'creator' && <Circle className='w-6 h-6 text-white fill-current' />}
                  {cell === 'creator-king' && <Crown className='w-6 h-6 text-yellow-400' />}
                  {cell === 'opponent' && <Circle className='w-6 h-6 text-white fill-current' />}
                  {cell === 'opponent-king' && <Crown className='w-6 h-6 text-yellow-400' />}
                </div>
              </button>
            ))}
          </div>
